...
- **The three enforcement scripts are env-overridable**, which is what makes the suite possible: `PARENTAL_CONFIG`, `PARENTAL_STATE_DIR`, `PARENTAL_WAN_IF`, `PARENTAL_LAN_IF`, `NFT`, `LOGGER`, `SLEEP`, `DATE`, plus `BLOCK_RULES_CONFIG` and `NSLOOKUP` for website blocking. New code should extend this pattern rather than hardcode paths. Not universal, though: `setup-adguard.sh` hardcodes its install paths and honours only `LOGGER`, and `panic-off.sh` honours only `NFT`. `PARENTAL_FIREWALL` is also a lever today but is being removed with the iptables backend, so do not build on it.
