...
- **A baseline reachability assertion is mandatory, not redundant.** Any topology failure (a dead server, a missing route, forwarding disabled) makes every probe read unreachable, which is indistinguishable from a perfectly blocking firewall, so a suite without a baseline assertion reports a flawless pass while testing nothing. That failure mode is the same class as the bug this decision exists to prevent, which is why the guard is structural rather than a nicety.
