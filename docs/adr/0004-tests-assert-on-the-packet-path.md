...
(Only these two were genuinely weighed. Nothing else was on the table, and no third option is listed here to manufacture deliberation that did not happen.)

## Consequences

- The test container needs `NET_ADMIN`, `SYS_ADMIN` and `NET_RAW` plus `net.ipv4.ip_forward=1` at start. `--privileged` is not required, so CI remains viable.
...
- The rule going forward, scoped deliberately: every **new or changed** enforcement claim gets a packet-path test; everything else may remain a unit test. This is not a retroactive demand on the whole suite. When this decision was taken, only the unknown-device allowlist was packet-proven; profile blocking, website blocking, tickets and budgets stay set-membership-only until the enforcement work reaches them. A reviewer should not block untouched work for lacking coverage this rule never asked for.
