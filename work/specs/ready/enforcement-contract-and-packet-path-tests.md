...
4. **Who owns the `websites` chain, and who creates it at boot?** The skeleton contains `jump websites`, but `website-blocking.sh` owns that chain. On a fresh boot the chain does not exist, so the jump fails, silently, into the `2>/dev/null` every nft call in this repo carries, leaving website blocking permanently unwired while the ruleset looks correct. Either the skeleton owner creates the empty chain, or the jump is added by whoever creates it.
5. **Which script owns boot restore, and what verb performs it?** Restoring persisted blocks has been assigned both to `setup-firewall.sh apply` and to the init script. It cannot be both: if `apply` restores, then every `install.sh` run and every manual `apply` silently re-imposes old blocks, which the `panic-off.sh` recovery path forbids. No restore subcommand exists in any script today and one must be named.
6. **How far does fail-loud extend?** Removing the `none` backend deletes today's only warning on a missing `nft`. `parental-profiles.sh` is covered below, but `setup-firewall.sh` (the sole owner of the skeleton AND the boot path) and `website-blocking.sh` still swallow every error and exit 0 printing success. Hardening one of three callers leaves the signature defect in the one that matters most at boot.
7. **What tracked change makes the `family` night block fire?** The defect (`0 24 * * *`, an hour cron never runs) exists only in `config/local/crontab`, which is gitignored, and the `family` profile exists only in `config/local/parental_profiles`. The shipped default schedules different profiles at valid hours. So there is no tracked file to fix. Options: validate cron hours in the installer's crontab merge and reject or correct out-of-range values; scrub the bad line during the merge; or drop the story and leave it to the operator. Currently it is promised by a user story and delivered by nothing.
...
11. As a visitor's device that belongs to no profile, I want to stay blocked from the internet, so that the allowlist keeps meaning something (this already works and must not regress).
...
jump websites                     # 4. per-profile website blocking  (owner: open question 4)
...
**`apply` reconciles derived sets and preserves live ones.** `allowed_macs` is DERIVED from the profiles config and must be reconciled against it, because preserving it blindly would let a device removed from config keep internet forever. `blocked_macs`, `ticket_macs` and `guest_macs` are LIVE RUNTIME state and must be preserved: `apply` may rebuild rules, it may not flush those three. It must also leave the `websites` chain and its per-pair children in place. This is the load-bearing sentence, because flushing `blocked_macs` at boot is precisely the bug being fixed.

**Website blocking uses one chain per profile+rule.** The `websites` chain contains only jumps to `websites_<profile>_<rule>` chains, each holding that pair's drop rules and owning its `blocked_sites_<profile>_<rule>` set. `disable <profile> <rule>` removes ONLY that chain, its jump, and its own set. A blanket flush of the shared chain is forbidden: the shipped default has three profiles disabling `no_streaming` at 08:00 in the same minute that gaming rules are enabled, so a shared flush would clear other profiles' live blocks. Creation and boot-time existence of the parent chain are open question 4.

**Block state is persisted outside tmpfs and carries a reason.** Per-profile block state moves to a persisted file under `/etc/config/` (path-based sysupgrade preservation is verified for that directory; backup inclusion is inferred and should be confirmed on the router). Its location must be overridable by an env lever alongside `PARENTAL_STATE_DIR`, so tests can isolate it and the boot-restore path can be exercised. The reason model itself is open question 1, and whether the usage counter joins it is open question 3.
...
| docs | stale-doc sweep: README (81 tests, `parental_websites`, the "auto-detected, iptables fallback" feature line), `docs/setup-guide.md`, `docs/example-5-profiles.md`, `config/block_rules.example`, `config/local/README.md`, and **`docs/architecture.md`** (which describes the iptables abstraction and lists `PARENTAL_FIREWALL` as a seam to extend, and which both task prompts tell builders to read first) |
