...
5. As a maintainer, I want the existing suite to keep passing on the new image, so that switching environments does not quietly drop coverage. Measured on the image before tasking: of 104 tests, 101 passed unchanged, 1 failed for a missing package (closed by installing it), and 2 iptables-backend cases are deliberately removed, giving 102 passing. One of those two removed cases had been passing vacuously, which is why the arithmetic is worth spelling out.
