...
`internet` is derived from set membership, in the chain's own order: `blocked (manual)` if the profile's MACs are in `manual_blocked_macs`, else `ticket` if they are in `ticket_macs`, else `blocked` if all are in `blocked_macs`, else `allowed` if none are, else `partial`. The `manual_blocked_macs` case is mandatory: without it a manually blocked profile reads as `allowed`, which is the enforcement-versus-state confusion this whole page exists to expose. Keeping **`partial` as an explicit state is deliberate**: it surfaces drift instead of hiding it behind a green dot, and drift of exactly that kind is what made the original enforcement bug invisible.
...
**Schedules must leave crontab.** Editing timings from a web form means rewriting cron lines from a web form, and cron cannot express what is wanted anyway. (This paragraph previously said `0 24 * * *` never fires because cron hours are 0-23. That was measured FALSE: busybox crond degrades an unparseable field to a wildcard, so the line fires every hour. See `work/notes/findings/busybox-crond-treats-a-bad-time-field-as-a-wildcard.md`.)

The precedence was also stated backwards here and is now settled the other way by `docs/adr/0006-a-block-carries-a-set-of-reasons-and-manual-outranks-a-ticket.md`: a **manual** block outranks a ticket, and a ticket outranks `budget` and `schedule`. The claim that the chain cannot express the middle tiers is likewise superseded: two block sets express it directly, with `manual_blocked_macs` above the ticket accept and `blocked_macs` below it. The reason set is still recorded in state, but for saying WHY a child is offline rather than because the chain cannot enforce the order.
