...
**The guest endpoint is a bedtime bypass unless it refuses registered devices.** In the enforcement chain `guest_macs accept` sits ABOVE `blocked_macs drop`, so a guest pass outranks a schedule block by design (a visitor should not be caught by a child's bedtime). That means a child who learns the household passphrase can grant their OWN registered device a pass from the public, unauthenticated endpoint and walk straight past bedtime. The registry check above is what closes it, which makes it a security control rather than a tidiness rule. The threat model names that child as the primary adversary, so this path must be covered before guest access ships.
